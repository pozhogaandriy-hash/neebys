...
/* ─────────────────────────────────────────────────────────────────────────────
   Validation helpers — all purely client-side, format/plausibility only.
   No bank or issuer verification is performed.
───────────────────────────────────────────────────────────────────────────── */
...
/**
 * Luhn algorithm — checks the check-digit of a card number.
 * Returns true when the number passes (plausible), false otherwise.
 */
...
/** Validate CVV length: 4 digits for Amex, 3 for all others. */
function validateCvv(raw: string, cardRaw: string): string | null {
  const digits = digitsOnly(raw);
  if (digits.length === 0) return 'CVV is required.';
  const expectedLength = isAmex(cardRaw) ? 4 : 3;
  if (digits.length !== expectedLength)
    return `CVV must be ${expectedLength} digits${isAmex(cardRaw) ? ' for Amex cards' : ''}.`;
  return null;
}

/**
 * Validate cardholder name: at least two words composed of letters,
...
function validateName(raw: string): string | null {
  ...
  if (!/^[A-Za-zÀ-ÿ'-]+([ ][A-Za-zÀ-ÿ'-]+)+$/.test(trimmed))
    return 'Enter your full name as it appears on the card (first and last name).';
  return null;
}

/** Format a raw digit string into groups separated by spaces (e.g. "4242 4242 4242 4242"). */
function formatCardDisplay(raw: string): string {
  const digits = digitsOnly(raw).slice(0, 19);
  return digits.replace(/(.{4})/g, '$1 ').trim();
}

/** Auto-format expiry input: insert "/" after two digits. */
...
/* ─────────────────────────────────────────────────────────────────────────────
   PaymentForm — Uiverse.io layout preserved; validation layer added.
───────────────────────────────────────────────────────────────────────────── */
...
function PaymentForm() {
  ...
  // Field values (controlled so we can auto-format)
  ...
  /** Run all validators and return the error map. */
  function validate(): FieldErrors {
    return {
      name: validateName(name) ?? undefined,
      cardNumber: validateCardNumber(cardNumber) ?? undefined,
      expiry: validateExpiry(expiry) ?? undefined,
      cvv: validateCvv(cvv, cardNumber) ?? undefined,
    };
  }

  /** Re-validate on change only after the user has already tried to submit. */
  function revalidate(overrides: Partial<{ name: string; cardNumber: string; expiry: string; cvv: string }>) {
    if (!submitted) return;
    const current = { name, cardNumber, expiry, cvv, ...overrides };
    setErrors({
      name: validateName(current.name) ?? undefined,
      cardNumber: validateCardNumber(current.cardNumber) ?? undefined,
      expiry: validateExpiry(current.expiry) ?? undefined,
      cvv: validateCvv(current.cvv, current.cardNumber) ?? undefined,
    });
  }
  ...
  const handleCheckout = (e: React.FormEvent) => {
    ...
    const errs = validate();
    setErrors(errs);
    if (Object.values(errs).some(Boolean)) return; // block if any error
    setDone(true);
  };

  if (done) {
    return (
      <div className="bg-white rounded-[26px] shadow-[0px_12px_26px_rgba(0,0,0,0.10),0px_47px_47px_rgba(0,0,0,0.09),0px_105px_63px_rgba(0,0,0,0.05)] w-full max-w-[450px] mx-auto">
        <div className="flex flex-col items-center gap-6 p-8 text-center">
          <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#1B1B1B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="20 6 9 17 4 12" />
            </svg>
          </div>
          <p className="text-[#1B1B1B] font-semibold text-base">Payment complete</p>
          <p className="text-[#8B8E98] text-sm">Thank you for your order. You will receive a confirmation shortly.</p>
          <button
            type="button"
            onClick={() => router.push('/')}
            className="h-[55px] w-full rounded-[11px] bg-gradient-to-b from-[#363636] via-[#1B1B1B] to-[#000000] text-white text-sm font-bold transition-all hover:shadow-[0px_0px_0px_2px_#FFFFFF,0px_0px_0px_4px_rgba(0,0,0,0.23)]"
          >
            Back to store
          </button>
        </div>
  ...
  const fieldBase =
    'h-10 pl-4 rounded-[9px] bg-[#F2F2F2] border outline-none transition-all duration-300 focus:shadow-[0px_0px_0px_2px_#242424] w-full text-[#1B1B1B] placeholder:text-[#AAABB0]';
  const fieldOk = 'border-transparent focus:border-transparent';
  const fieldErr = 'border-red-400 focus:border-red-400 focus:shadow-[0px_0px_0px_2px_#f87171]';

  return (
    <div className="bg-white rounded-[26px] shadow-[0px_187px_75px_rgba(0,0,0,0.01),0px_105px_63px_rgba(0,0,0,0.05),0px_47px_47px_rgba(0,0,0,0.09),0px_12px_26px_rgba(0,0,0,0.10),0px_0px_0px_rgba(0,0,0,0.10)] w-full max-w-[450px] mx-auto">

      <form onSubmit={handleCheckout} noValidate className="flex flex-col gap-5 p-5">

        {/* Payment method buttons */}
        <div className="grid grid-cols-3 gap-5 p-[10px]">
          {/* PayPal */}
          <button
            name="paypal"
            type="button"
            className="h-[55px] bg-[#F2F2F2] rounded-[11px] flex items-center justify-center outline-none border-0 transition-opacity hover:opacity-75"
            aria-label="Pay with PayPal"
          >
            <svg xmlSpace="preserve" viewBox="0 0 124 33" height="18" width="auto" id="Layer_1" xmlns="http://www.w3.org/2000/svg">
          ...
          >
            <svg xmlSpace="preserve" viewBox="0 0 512 210.2" height="22" width="auto" xmlns="http://www.w3.org/2000/svg">
          ...
          >
            <svg fill="none" viewBox="0 0 80 39" height="22" width="auto" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* Separator */}
        <div className="grid gap-[10px] text-[#8B8E98] mx-[10px]" style={{ gridTemplateColumns: '1fr 2fr 1fr' }}>
          <hr className="border-0 bg-[#e8e8e8] h-px my-auto" />
          <p className="text-center font-semibold text-[11px] break-keep m-auto">or pay using credit card</p>
          <hr className="border-0 bg-[#e8e8e8] h-px my-auto" />
        </div>
        ...
        <div className="flex flex-col gap-[15px]">
          ...
          <div className="flex flex-col gap-[5px]">
            <label htmlFor="card-holder" className="text-[10px] text-[#8B8E98] font-semibold">
              Card holder full name
            </label>
            <input
              id="card-holder"
          ...
          {/* Card number */}
          <div className="flex flex-col gap-[5px]">
            <label htmlFor="card-number" className="text-[10px] text-[#8B8E98] font-semibold">
              Card Number
            </label>
            <input
              id="card-number"
              ...
              onChange={(e) => {
                ...
                revalidate({ cardNumber: formatted });
              }}
              className={`${fieldBase} ${errors.cardNumber ? fieldErr : fieldOk} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
            />
            {errors.cardNumber && (
              <p className="text-[11px] text-red-500 mt-0.5">{errors.cardNumber}</p>
            )}
          </div>

          {/* Expiry + CVV */}
          <div className="flex flex-col gap-[5px]">
            <label htmlFor="expiry" className="text-[10px] text-[#8B8E98] font-semibold">
              Expiry Date / CVV
            </label>
            <div className="grid gap-[15px]" style={{ gridTemplateColumns: '4fr 2fr' }}>
              <div className="flex flex-col gap-[5px]">
                <input
                  id="expiry"
                  type="text"
                  ...
                  onChange={(e) => {
                    ...
                    setExpiry(formatted);
                    revalidate({ expiry: formatted });
                  }}
                  className={`${fieldBase} ${errors.expiry ? fieldErr : fieldOk}`}
                />
                {errors.expiry && (
                  <p className="text-[11px] text-red-500 mt-0.5">{errors.expiry}</p>
                )}
              </div>
              <div className="flex flex-col gap-[5px]">
                <input
                  ...
                  value={cvv}
                  onChange={(e) => {
                    const val = digitsOnly(e.target.value).slice(0, 4);
                    setCvv(val);
                    revalidate({ cvv: val });
                  }}
                  className={`${fieldBase} ${errors.cvv ? fieldErr : fieldOk} [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                />
                {errors.cvv && (
                  <p className="text-[11px] text-red-500 mt-0.5">{errors.cvv}</p>
                )}
              </div>
            </div>
          </div>
        ...
        <button
          type="submit"
          className="h-[55px] rounded-[11px] border-0 outline-none text-white text-[13px] font-bold bg-gradient-to-b from-[#363636] via-[#1B1B1B] to-[#000000] transition-all duration-300 hover:shadow-[0px_0px_0px_2px_#FFFFFF,0px_0px_0px_4px_rgba(0,0,0,0.23)] w-full"
        >
          Checkout
        </button>

        {/* Disclaimer */}
        <p className="text-center text-[10px] text-[#AAABB0]">
          Card details are validated for format only. No payment is processed.
        </p>
      </form>
    </div>
  );
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: "var(--gf-bg)", color: "var(--gf-text)" }}>
      ...
      <div className="max-w-[1440px] mx-auto px-6 pt-32 pb-24">
        <h1 className={`${headingFont.className} text-4xl md:text-6xl uppercase tracking-[0.15em] mb-16`}>
          ОПЛАТА
        </h1>
